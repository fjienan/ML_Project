...
为进一步验证融合模型的鲁棒性，本文绘制了纯NB、融合模型与纯TAN的多分类ROC曲线（Micro-AUC），如图 \ref{fig:roc_comparison} 所示。
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/classification/roc_comparison.png}
\caption{纯NB vs 融合模型 vs 纯TAN — ROC曲线对比}
\label{fig:roc_comparison}
\end{figure}

曲线显示，融合模型的Micro-AUC为0.8642，显著高于纯TAN的0.8002，表明其在概率排序与类别区分能力上更优。尽管纯NB的AUC（0.9083）最高，但其准确率仅为0.824，说明其概率排序能力虽强，但最终硬分类决策的泛化能力不足。融合模型在保持与纯TAN相同准确率（0.882）的同时，显著提升了概率输出的可靠性，实现了准确率与鲁棒性的双重优势。
